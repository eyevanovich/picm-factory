...
13. In coding additive mode, preserve existing documentation and add only missing routing/maps. In curated mode, draft a documentation consolidation proposal with canonical-source recommendations and separately highlighted merges, moves, archive candidates, rewrites, and deletions; never treat curated selection as blanket write approval. A declined proposal remains no-write: after its inspection phase is ended, do not retry a gate-blocked read or reopen a scan merely to revisit it; call `complete` before reporting.
