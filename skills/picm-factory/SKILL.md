...
- **Specialist Folder:** derive the final guidance only from the approved generated routes. Include a `specialistFirstRun` object in the approved `.picm/config.json` with the exact first recipe path, all inputs, expected artifact, `requiresInspectEditApprove: true`, next-action source, and every visible uncertainty category. After the approved config and named recipe are written, call `picm_specialist_first_run_guidance` with no arguments and use its returned text as the final first-run guidance. Do not require recipes to use particular headings or wording. After review, distinguish one-off artifact edits from durable lessons that belong in an approved existing `rules.md`, `examples.md`, or `reference/` route; never invent optional folders, recipes, or operations. Recommend `/picm-maintain` after the first real use or when the specialist workflow, routing, or stable guidance changes.
