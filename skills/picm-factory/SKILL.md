...
3. Apply `/picm-new` safety.
   ...
   - If existing architecture is present, recommend `/picm-adopt`. Protected inventory marks this as `newWorkflowIntentRequired`. End the discovery phase, ask the user to choose `adopt existing`, `add/replace scaffold`, or cancel, and keep the privacy-reviewed workflow open while awaiting the direct answer. Record that answer with `picm_scan_control` action `new-intent` (`adopt-existing`, `add-replace`, or `cancel`) before `complete`. Add/replace starts a new protected phase before any project reads, drafting, preview, or registered scaffold writes; adopt existing continues the same authorization as adoption and starts a new protected phase before project reads. A preview-only or vague reply remains no choice and strict no-write.
