...
1. Run `picm_scan_control preflight` before inspecting anything. For routine maintenance, preflight automatically loads persisted `.picm/config.json` exclusions. If it reports `privacyReviewed: true`, ask only for any additional sensitive project-relative paths, call `privacy` with those additions (or an empty list for `none`), and retain every persisted exclusion without repeating the full boilerplate. If it reports `privacyReviewed: false`, ask the full security/privacy question and record exact exclusions with `privacy`. Then call `begin`.
