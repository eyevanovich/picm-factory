...
3. If the repo uses the Coding Repository profile, `capabilities.codebaseMap`, or visible `CONTEXT-MAP.md`, load `references/coding-maintenance-rubric.md`. Apply its complete protected-scan boundary before any coding inspection. For an interactive run, apply only the Strict or Balanced one-run depth supplied by the command prompt; never mutate the stored preset. Scheduled automatic maintenance honors an explicit stored `light`, `balanced`, or `strict` value and falls back to Balanced when the historical value is missing.
