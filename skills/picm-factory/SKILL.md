...
10. Before offering any adoption write, if protected inventory found sensitive material in a non-Git workspace, give actionable protection guidance alongside repository-visibility and context-boundary guidance: propose exact `.gitignore` patterns for eventual commit protection (or explain the equivalent PiCM session/persisted exclusions), ask the owner to confirm appropriate workspace/repository visibility, and keep sensitive source outside reusable context, examples, and adoption metadata. Never initialize Git or modify `.gitignore` without direct approval.
