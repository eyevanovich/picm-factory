...
4. **Preview before writes.** For adoption, maintenance, and optimization, apply `references/preview-review-protocol.md` before every proposed project write: show a complete concise summary, require mandatory exact review for deletions and changes to safety, permissions, approval boundaries, or required commands, then accept a direct explicit approval of the current summary before writing the exact proposal. Draft adjustments preserve applicable unchanged-path review state, supersede pending write approval, and refresh the current proposal before direct approval. Preserve the existing exact confirmations for persisted privacy exclusions and standalone maintenance-policy writes. For new scaffolds, show planned file changes before applying them, including brand-new scaffolds in confirmed empty-enough workspaces.
   Initial adoption creation of ordinary `.picm/config.json` and `.picm/adoption-report.md` is never itself a mandatory exact-review item; both remain available for on-demand review.
...
1. Run `picm_scan_control preflight` before inspecting anything. For routine maintenance, preflight automatically loads persisted `.picm/config.json` exclusions. If it reports `privacyFollowupPending: true`, ask only for any additional sensitive project-relative paths, call `privacy` with those additions (or an empty list for `none`), and retain every persisted exclusion without repeating the full boilerplate. Otherwise, ask the full security/privacy question and record exact exclusions with `privacy`. Then call `begin`.
...
14. Offer edits only through `references/preview-review-protocol.md`: before every proposed project write, present its complete concise summary; require exact review for deletions and changes to safety, permissions, approval boundaries, or required commands; then accept direct explicit approval of the current summary. Draft adjustments preserve applicable unchanged-path review state, refresh the current proposal, and never auto-heal.
15. Offer to write `.picm/maintenance-report.md` only through the same summary-preview, mandatory-review, and direct-approval gate.
...
- Adoption, maintenance, and optimization use a complete concise summary before every proposed project write, selective exact review when requested, mandatory exact review for deletions and safety/permission/approval-boundary/required-command changes, and direct explicit approval of the current summary. Adoption and optimization are non-destructive by default and do not rename, move, rewrite, or restructure existing files without approval of the exact action.
