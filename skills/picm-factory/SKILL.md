...
4. **Preview before writes.** For adoption, maintenance, and optimization, apply `references/preview-review-protocol.md` before every proposed project write: show a complete concise summary, require mandatory exact review for deletions and changes to safety, permissions, approval boundaries, or required commands, then accept a direct explicit approval of the current summary before writing the exact proposal. Proposal revisions invalidate earlier preview/review/approval. Preserve the existing exact confirmations for persisted privacy exclusions and standalone maintenance-policy writes. For new scaffolds, show planned file changes before applying them, including brand-new scaffolds in confirmed empty-enough workspaces.
