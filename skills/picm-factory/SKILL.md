...
3. Apply `/picm-new` safety.
   ...
   - If existing architecture is present, recommend `/picm-adopt`. If the user insists on `/picm-new`, require them to choose the intent (`adopt existing` vs `add/replace scaffold`) and use the same current-exact-proposal gate: preview and directly approve every create/update/overwrite action before writing it. A preview-only or vague reply remains a strict no-write outcome.
