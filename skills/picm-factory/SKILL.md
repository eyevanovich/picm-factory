...
1. Run `picm_scan_control preflight` before inspecting anything. For routine maintenance, preflight automatically loads persisted `.picm/config.json` exclusions and completes privacy review without an interview. If it reports `privacyReviewed: false`, ask the security/privacy question, record exact exclusions with `privacy`, then call `begin`.
