...
4. **Preview before writes.** For adoption, maintenance, and optimization, apply `references/preview-review-protocol.md` before every proposal batch: show a complete concise summary, flag material, uncertain, linked, or deletion changes with non-blocking review suggestions, then accept direct explicit approval of the current summary before writing the exact proposal. Draft adjustments preserve applicable unchanged-path review state, supersede pending write approval, and refresh the current proposal before direct approval. For `/picm-adopt` and `/picm-maintain`, prepare exact create, modify, delete, and linked-move operations with `picm_proposal_batch` during an active protected scan, then use it to apply only the directly approved current batch; it records a session audit or reports a post-apply audit warning, rechecks protected paths and expected content, and stops later operations after failure or cancellation without undoing completed files or created parents. Report its completed, unattempted, failed, or uncertain per-operation results, including a published move destination, without agent Bash. Preserve the separate runtime confirmations for persisted privacy exclusions and standalone maintenance-policy writes. For a standalone policy, the accepted complete summary remains no-write; only its following exact TUI patch confirmation may apply the accepted preview. For new scaffolds, register and show one current exact proposal with every planned create, update, append, and overwrite action, including brand-new scaffolds in confirmed empty-enough workspaces. A `preview only` reply or vague assent—including `continue`, `looks good`, `yes`, `go ahead`, or a lone `.`—is a strict no-write outcome: keep the proposal pending, state that nothing was written, and invite one of the explicit approval forms listed in step 10. Apply all and only the registered actions after one of those forms approves that current exact proposal.
...
9. **Recommend a user-created Git checkpoint, never a Git action.** Before PiCM writes, combine the current exact preview with the checkpoint guidance in `references/preview-review-protocol.md`. Use permitted preflight metadata or user confirmation only; do not inspect Git status, history, or file contents to verify coverage. Never initialize, stage, commit, reset, clean, or restore Git for the user.
...
11. **Scheduled maintenance stays advisory and reminder-based.** Maintenance can remain manual or configure scheduled maintenance reminders with a cadence in days, weeks, or months. Scheduling requires `.picm/config.json` to be non-ignored and a regular non-symlink file under a regular non-symlink `.picm/` directory. When maintenance is due in an interactive TUI session, a persistent reminder renders above the editor with Run Now and Defer options. Defer dismisses the reminder for the current session only. Run Now invokes the ordinary privacy-reviewed maintenance flow and depth selection. Timestamps advance and the reminder clears only after successful maintenance completion; adoption, declining an initial pass, and stopping before the completion settings are published leave configured reminders unchanged. Once those settings are published, late cancellation does not undo them. Follow the settings-publication guidance in `references/preview-review-protocol.md` for committed results and durability warnings. Existing manual, nudge, and automatic configurations remain parseable; legacy scheduled modes use this approval prompt without unconfirmed automatic execution. Nothing runs while Pi is closed or in print, JSON, RPC, or headless modes. For a standalone policy write, pass the `previewId` returned by `picm_maintenance_policy` preview into apply so the confirmed patch reuses the exact previewed timestamps.
...
- `references/coding-maintenance-rubric.md` — Strict/Balanced run depths and legacy codebase-map metadata guidance.
...
3. Apply `/picm-new` safety.
   ...
   - Source-material-only: folders/files such as `src/`, `docs/`, `notes/`, `assets/`, `data/`, transcripts, briefs, screenshots, or domain documents. Ask whether to build around them without moving or rewriting them. For identified sensitive or local-only paths, offer exact optional `.gitignore` entries (for example, `/notes/local-only.md`) for commit protection, even when no root `.gitignore` exists. Explain that session scan exclusions do not prevent Git commits; declining the ignore proposal leaves files unchanged.
   ...
   - If existing architecture is present, recommend `/picm-adopt`. Protected inventory marks this as `newWorkflowIntentRequired`. End the discovery phase, ask the user to choose `adopt existing`, `add/replace scaffold`, or cancel, and keep the privacy-reviewed workflow open while awaiting the direct answer. Record only that directly observed answer with `picm_scan_control` action `new-intent` (`adopt-existing`, `add-replace`, or `cancel`) before `complete`; it rejects any mismatched or vague input. Do not begin another phase while the choice is pending. Add/replace starts a new protected phase before any project reads, drafting, preview, or registered scaffold writes; adopt existing continues the same authorization as adoption and starts a new protected phase before project reads. A preview-only or vague reply remains no choice and strict no-write.
...
4. Apply the Git checkpoint recommendation with the current exact scaffold preview.
   - Strongly recommend that the user create a Git commit covering the current contents of affected existing files. A repository or old commit does not protect current uncommitted work; a checkpoint does not protect uncommitted or untracked work, and broad Git restore can erase newer edits.
   - Do not inspect Git status, history, or file contents to verify coverage; do not require a repository-wide clean state or ask the user to add sensitive or ignored material. Never initialize, stage, commit, reset, clean, or restore Git for the user.
   - Non-Git and new/empty workspaces remain supported. A first post-scaffold commit protects future contents only. When the exact scaffold affects current existing content and coverage is absent or uncertain, obtain the explicit risk opt-out or a clear checkpoint report required by `references/preview-review-protocol.md`; either is an unverified acknowledgment, not scaffold approval. New-only scaffolds remain directly approvable.
...
9. Keep a protected phase active through scaffold preview, review, approval, and writes. If discovery was ended for the interview, call `begin` before registering the proposal; call `end` only after applying or declining it. Register one current exact scaffold proposal with `picm_scaffold_proposal` action `preview`, including the complete exact `write` or `edit` tool input for every file create, append, update, and overwrite. Present its returned preview ID and enumerated actions, including exact append/update proposals for existing safe files such as `README.md` or `.gitignore`. In a new scaffold config preview, show `"createdAt": "{{createdAt}}"` and explain that the approved write resolves only this marker to the write-time canonical ISO 8601 UTC timestamp. This is a value resolution in the already-reviewed `.picm/config.json` write, not an additional file or write action. If the user replies `preview only`, gives vague assent, or sends a lone `.`, do not write; retain the proposal and state that it remains pending direct approval.
10. Write all and only that registered proposal after the user replies with one of these explicit approval forms (case-insensitive): `approve this exact scaffold`, `accept the current proposal and write it`, `write exactly this proposal`, or `I approve the current exact proposal; write it now`. Any requested change invalidates the registered proposal, creates a newly registered proposal, and requires a refreshed preview and direct approval.
...
- Do not write unresolved `{{picm:...}}` tokens into generated files; any case-sensitive `{{picm:` occurrence is unresolved.
...
- **Specialist Folder:** use the authoritative visible-recipe receipt contract in `references/layout-profiles.md`; do not infer routes from recipe prose. Keep the local `paths.firstRecipe` hint and any applicable legacy input hints aligned with that receipt. Write the config only after the completed visible scaffold and recipe writes, then call `picm_specialist_first_run_guidance` with no arguments and use its returned text as the final first-run guidance. The runtime checks approved scaffold files only where the receipt requires it. After review, distinguish one-off artifact edits from durable lessons that belong in an approved existing `rules.md`, `examples.md`, or `reference/` route; never invent optional folders, recipes, or operations. Recommend `/picm-maintain` after the first real use or when the specialist workflow, routing, or stable guidance changes.
...
6. For coding adoption, treat shallow classification and initial orientation as one scan phase: call `end` before asking for the mapping approach (**root**, **distributed**, or **scan and recommend**), adoption depth (**additive** or **curated**), and optional user boundary hints. Do not ask for a maintenance depth. After the mapping and adoption-depth choices, call `begin` again before any project or resource read for the Strict examination, map analysis, or Curated documentation inventory. Keep that inspection phase active while preparing, presenting, and resolving its proposal, then end it cleanly. Automatically perform the Strict adoption examination: broader systematic coverage across declared roots and mapped contexts at higher cost.
...
11. If routing is adequate, offer minimal `.picm/` adoption metadata after explicit confirmation. Coding metadata records only the resulting root/distributed shape, roots, map path/equivalent, selected local contexts, and `maintenancePreset: "strict"`. This legacy metadata records the Strict-first adoption baseline; it does not select a future maintenance run depth or grant future writes.
...
13. In coding additive mode, preserve existing documentation and add only missing routing/maps. In curated mode, draft a documentation consolidation proposal with canonical-source recommendations and separately highlighted merges, moves, archive candidates, rewrites, and deletions; never treat curated selection as blanket write approval. A declined proposal remains no-write: cancel it while the inspection phase is active, end the phase, and call `complete` before reporting; do not retry a gate-blocked read or reopen a scan merely to revisit it.
...
4. If the repo uses the Coding Repository profile, `capabilities.codebaseMap`, or visible `CONTEXT-MAP.md`, load `references/coding-maintenance-rubric.md`. Apply its complete protected-scan boundary before any coding inspection. Apply only the Strict or Balanced one-run depth supplied by the command prompt, including after a scheduled `Run Now`; a bare non-TUI command defaults to Strict without opening a selector. Never mutate `capabilities.codebaseMap.maintenancePreset`. Historical `light`, `balanced`, and `strict` values remain preserved and readable as legacy metadata, but never select a run depth; a missing value has no Balanced fallback. Light has no active or scheduled compatibility dispatch and is not offered in the selector or new adoption.
...
- pinned public npm project-local install:
  `pi install -l npm:@eyevanovich/picm-factory@0.4.0`
...
- After an adoption writes exactly `adoption.status: "adopted"`, PiCM asks “Would you like to run an initial maintenance pass now (recommended)?” with `Run maintenance now` and `Finish`; other adoption outcomes finish without this offer. A same-conversation run reuses confirmed exclusions, then uses the same Strict-preselected selector; Finish or stopping before completion settings are published leaves maintenance records and reminders unchanged; late cancellation after publication does not undo the saved settings. A later conversation performs normal maintenance privacy review.
...
- The interactive choice applies to one run only and never silently changes `capabilities.codebaseMap.maintenancePreset`; a bare non-TUI command defaults to Strict without opening a selector. Historical `light`, `balanced`, and `strict` values remain preserved and readable as legacy metadata, but never select a run depth; a missing value has no Balanced fallback. Light has no active or scheduled compatibility dispatch and is not offered for new adoption or interactive run selection.
