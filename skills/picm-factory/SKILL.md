...
4. **Preview before writes.** For adoption, maintenance, and optimization, apply `references/preview-review-protocol.md` before every proposed project write: show a complete concise summary, require mandatory exact review for deletions and changes to safety, permissions, approval boundaries, or required commands, then require separate explicit approval for the current proposal. Proposal revisions invalidate earlier preview/review/approval. Preserve the existing exact confirmations for persisted privacy exclusions and standalone maintenance-policy writes. For new scaffolds, show planned file changes before applying them, including brand-new scaffolds in confirmed empty-enough workspaces.
...
13. Before every proposed project write, apply `references/preview-review-protocol.md`: present its complete concise summary, finish all mandatory exact review, and obtain separate explicit approval for the current proposal. Option or cadence choice, preview request, review navigation, or vague assent is not approval. Any revision invalidates prior acceptance/review/approval and requires a refreshed summary. Then write only the exact approved changes, including for `.picm/` files.
...
1. Run `picm_scan_control preflight`, ask the security/privacy question, record exact exclusions with `privacy`, and call `begin` before inspecting anything.
...
3. If the repo uses the Coding Repository profile, `capabilities.codebaseMap`, or visible `CONTEXT-MAP.md`, load `references/coding-maintenance-rubric.md`. Apply its complete protected-scan boundary before any coding inspection. For an interactive run, apply only the Strict or Balanced one-run depth supplied by the command prompt; never mutate the stored preset. Historical stored `light`, `balanced`, or `strict` values remain readable and preset-driven scheduled maintenance honors them; a missing stored value falls back to Balanced.
...
13. Keep the entire run read-only and advisory: chat findings only, with no unauthorized scan lifecycle action, agent-initiated Bash, edits, report, repair, commit, or external side effect. User-typed `!bash` remains unrestricted. The extension's schedule timestamp reset occurs only upon successful completion and authorizes no other write.
14. Offer edits only through `references/preview-review-protocol.md`: before every proposed project write, present its complete concise summary; require exact review for deletions and changes to safety, permissions, approval boundaries, or required commands; then obtain separate explicit approval for the current proposal. Revisions invalidate earlier acceptance/review/approval. Never auto-heal.
15. Offer to write `.picm/maintenance-report.md` only through the same summary-preview, mandatory-review, and separate-approval gate.
...
- `/picm-optimize` — inspects agent-facing documentation and offers selected outcome-preserving improvements.
...
- Adoption, maintenance, and optimization use a complete concise summary before every proposed project write, selective exact review when requested, mandatory exact review for deletions and safety/permission/approval-boundary/required-command changes, and separate explicit approval for the current proposal. Adoption and optimization are non-destructive by default and do not rename, move, rewrite, or restructure existing files without approval of the exact action.
