...
- **Specialist Folder:** derive the final guidance only from the approved generated routes. Record the exact first recipe path as `paths.firstRecipe` and only planned generated reusable input files as `paths.generatedInputs` in the approved `.picm/config.json`; runtime/future inputs are route references and do not belong in that list. Write the config only after the completed visible scaffold and recipe writes. Then call `picm_specialist_first_run_guidance` with no arguments and use its returned text as the final first-run guidance; the runtime parses the persisted approved recipe into normalized route roles and validates each planned generated input against approved writes. Recipes may use `Inputs`, `Sources`, or `What it reads`; `Expected artifact`, `Output`, `Expected output`, or `Result`; and `Review gate and next action`, `Review gate`, `Review and next action`, or `Approval and handoff` headings. After review, distinguish one-off artifact edits from durable lessons that belong in an approved existing `rules.md`, `examples.md`, or `reference/` route; never invent optional folders, recipes, or operations. Recommend `/picm-maintain` after the first real use or when the specialist workflow, routing, or stable guidance changes.
