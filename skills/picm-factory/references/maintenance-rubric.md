...
When the Coding Repository profile, `capabilities.codebaseMap`, or visible `CONTEXT-MAP.md` is present, load `coding-maintenance-rubric.md`. Its Strict/Balanced run depths and legacy codebase-map metadata guidance extend this rubric, and its privacy-first exclusion boundary applies before any coding scan.
...
Never apply repairs automatically. For every proposal batch, use `preview-review-protocol.md`: present the complete concise summary with non-blocking review suggestions and wait for direct explicit approval of the current summary. During an active protected scan, prepare the exact create, modify, delete, and linked-move operations with `picm_proposal_batch`; only its directly approved `apply` writes the current batch, stops later operations after a failure or cancellation without undoing completed files or created parents, reports per-operation partial results, and records the audit without agent Bash. A draft adjustment supersedes pending write approval while preserving applicable unchanged-path review state. Offer edits only in `Changes I can apply with your approval`.

Scheduled maintenance is reminder-based: when `nextDueAt` arrives, the first eligible interactive TUI session renders a persistent reminder above the editor and presents `Run Now` and `Defer` options, rather than launching an unconfirmed run or running while Pi is closed. Choosing `Defer` dismisses the reminder for the current session only. Choosing `Run Now` starts the ordinary privacy-reviewed maintenance flow with Strict/Balanced depth selection; it grants no approval for a report, repair, commit, or external side effect, so every proposed project write still follows the normal preview, review, and approval gates. User-typed `!bash` remains unrestricted. Advancing `lastCycleAt`/`nextDueAt` and clearing the reminder occurs only after successful completion of maintenance. Cancellation, failure, or a cycle-reset configuration conflict before completion settings are published leaves maintenance due; resolve the cause and retry `picm_scan_control complete`. Once completion settings are published, maintenance remains completed despite late cancellation, and a directory-sync failure is a committed durability warning rather than an incomplete reset to retry. Existing manual, nudge, and automatic configurations remain parseable and both legacy scheduled modes use this approval prompt. Maintenance may run with or without `.git`; when repository metadata is absent the privacy-reviewed flow uses the extension's transient isolated Git view to honor `.gitignore` without modifying the workspace.
