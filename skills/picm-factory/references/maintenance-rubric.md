...
Never apply repairs automatically. For every proposal batch, use `preview-review-protocol.md`: present the complete concise summary with non-blocking review suggestions and wait for direct explicit approval of the current summary. A draft adjustment supersedes pending write approval while preserving applicable unchanged-path review state. Offer edits only in `Changes I can apply with your approval`.
