...
- Direct summary approval: option/cadence choices, preview requests, review navigation, and vague assent do not approve writes. Draft adjustments supersede pending write approval while preserving applicable review state for unchanged paths. Each refreshed summary invites the user to approve directly or inspect a diff. An unambiguous `accept`, `approve`, `accept and write`, or `proceed` directly authorizes the current summary. For `/picm-adopt` and `/picm-maintain`, the protected `picm_proposal_batch` tool applies only that prepared create/modify/delete/linked-move batch, rechecks its paths and expected content, rolls back a failed mutation, and records a session audit without agent Bash. For a standalone maintenance-policy preview, summary acceptance remains no-write; the following exact TUI patch confirmation alone controls application.
