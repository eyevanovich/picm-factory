...
- an automatic Strict initial examination, recorded in `capabilities.codebaseMap.maintenancePreset: "strict"` as legacy metadata without a depth question;
- hybrid workspaces where codebase mapping overlaps Stage Pipeline, Specialist Folder, Team / Role OS, or custom layouts.

After an adoption that successfully writes `adoption.status: "adopted"`, PiCM asks exactly once: “Would you like to run an initial maintenance pass now (recommended)?” with **Run maintenance now** and **Finish**. Already-adopted, preview-only, declined, cancelled, failed, scanned-only, needs-routing, missing-status, and malformed-status outcomes finish without offering initial maintenance. Running it reuses that conversation's confirmed privacy exclusions, then enters the ordinary Strict-preselected depth selector and profile-appropriate maintenance flow. Finishing without maintenance or stopping before its completion settings are published leaves maintenance records and configured reminders unchanged; late cancellation after publication does not undo the saved settings. A later conversation uses normal maintenance privacy review.

Every later interactive `/picm-maintain` run selects a one-run depth with Strict preselected. `/picm-maintain strict` and `/picm-maintain balanced` bypass the selector. In non-TUI dispatch, a bare command defaults to Strict without opening a selector. The run choice never silently changes the stored metadata. At maintenance intake, PiCM asks whether to include agent-document optimization and defaults to No. No runs standard maintenance unchanged; Yes reuses the established documentation-only optimization scope, outcome-preservation checks, selectable proposals, no-worthwhile-change outcome, privacy boundary, and shared preview/review safeguards.
...
Historical `capabilities.codebaseMap.maintenancePreset` values (`light`, `balanced`, and `strict`) remain preserved and readable as legacy metadata, but never select a maintenance run depth. A missing value has no Balanced fallback. Light has no active or scheduled compatibility dispatch and is not offered in new adoption or interactive selectors.
...
The extension stores explicit UTC `lastCycleAt` and `nextDueAt` timestamps. Existing manual, nudge, and automatic config values remain parseable. When maintenance is due in an interactive TUI session, PiCM renders a persistent reminder section above the editor and presents a selector with `Run Now` and `Defer`. `Defer` dismisses the reminder for the current session only and asks again in fresh sessions. `Run Now` invokes the ordinary privacy-reviewed maintenance flow and Strict/Balanced depth selector, not a stored maintenance preset. `lastCycleAt` and `nextDueAt` advance and the reminder clears only upon successful maintenance completion; adoption, declining an initial pass, or stopping before completion settings are published leaves configured reminders unchanged. A settings save commits when the prepared config replaces `.picm/config.json`: late cancellation does not undo it. A subsequent directory-sync failure reports that the settings were saved but survival of a crash is uncertain, rather than asking to repeat the reset. Nothing runs while Pi is closed or in print, JSON, RPC, or other headless execution.

Legacy `config.json.rollback-*` files from older versions are left untouched. PiCM no longer creates cancellation snapshots or automatically restores them. Preserve any copy you need and compare it before choosing a targeted recovery or cleanup; a stale snapshot can overwrite newer settings.
...
- Direct summary approval: option/cadence choices, preview requests, review navigation, and vague assent do not approve writes. For `/picm-new`, even `continue`, `looks good`, `yes`, `go ahead`, or a lone `.` writes nothing; only one of the explicit approval forms shown with the current exact scaffold proposal authorizes all and only its registered actions. For adoption, maintenance, and optimization, an unambiguous `accept`, `approve`, `accept and write`, or `proceed` authorizes the current summary. Draft adjustments supersede pending write approval while preserving applicable review state for unchanged paths. Each refreshed summary invites the user to approve directly or inspect a diff. For `/picm-adopt` and `/picm-maintain`, the protected `picm_proposal_batch` tool deterministically generates and records delivery of the prepared batch's ID, digest, exact operations and contents, and approval prompt before accepting those tokens, then applies only that create/modify/delete/linked-move batch, rechecks its paths and expected content, and on failure or cancellation stops later operations without undoing completed files or created parents. It returns completed, unattempted, failed, or uncertain per-operation results, including any published move destination, and records a session audit without agent Bash. If a post-apply audit cannot be stored, it returns an audit warning without hiding file effects. For a standalone maintenance-policy preview, summary acceptance remains no-write; the following exact TUI patch confirmation alone controls application.
