...
- Direct summary approval: option/cadence choices, preview requests, review navigation, and vague assent do not approve writes. Draft adjustments supersede pending write approval while preserving applicable review state for unchanged paths. Each refreshed summary invites the user to approve directly or inspect a diff. An unambiguous `accept`, `approve`, `accept and write`, or `proceed` directly authorizes the current summary. For `/picm-adopt` and `/picm-maintain`, the protected `picm_proposal_batch` tool deterministically generates and records delivery of the prepared batch's ID, digest, exact operations and contents, and approval prompt before accepting those tokens, then applies only that create/modify/delete/linked-move batch, rechecks its paths and expected content, rolls back a failed mutation, and records a session audit without agent Bash. For a standalone maintenance-policy preview, summary acceptance remains no-write; the following exact TUI patch confirmation alone controls application.
