...
Explain command choice in plain language without assuming PiCM/ICM jargon: new or mostly empty workflow → `/picm-new`; existing source-code, agent/workflow, or Claude-style folder → `/picm-adopt`; known repository/monorepo → optional `/picm-adopt coding` shortcut, while regular `/picm-adopt` can offer the same Coding Repository profile; workspace health or drift → `/picm-maintain`; one concrete symptom → `/picm-maintain trace "describe what drifted"`; repetitive, diffuse, or hard-to-navigate agent-facing documentation → `/picm-optimize`. Include privacy-first protected coding scans, project-local install, `.pi/` versus `.picm/`, and non-destructive adoption defaults. Explain that adoption, maintenance, and optimization show a complete concise summary preview before every proposed project write, offer selective exact review, require exact review for deletions and safety/permission/approval-boundary/required-command changes, and require separate explicit approval for the current proposal. Explain that optimization is documentation-only, preserves every unique constraint, and does not claim semantic equivalence or guaranteed token savings.
