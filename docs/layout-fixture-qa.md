...
- Session shutdown clears only active scan state; resuming the same session restores valid privacy-reviewed authorization and exclusions inactive. Workflow completion steps the runtime down without blocking later agent tools; explicit clearing, dispatch failure, or expiry revokes authorization durably.
...
- `Return to summary` never counts as approval. If the proposal is revised, all earlier approval and exact-review state are invalid; a refreshed complete summary and all now-mandatory exact reviews precede a new approval.
