...
function buildPrompt(
  ...
  const adoptionRoutingGuidance = command === "picm-adopt"
    ? "\n\nWhen neither `AGENTS.md` nor `CLAUDE.md` exists, recommend `AGENTS.md` as the default routing source and, before presenting the final proposal, explicitly ask whether to draft a small `CLAUDE.md` compatibility shim. An accepted shim appears only in the draft; declining it leaves the remainder of the proposal unchanged. This choice does not authorize writes: write neither file until the user directly approves the final proposal."
    : "";
